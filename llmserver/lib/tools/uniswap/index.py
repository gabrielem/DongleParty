...
from cdp_agentkit_core.actions.wow.uniswap.constants import (
    UNISWAP_QUOTER_ABI,
    UNISWAP_V3_ABI,
)
...
def get_uniswap_quote(
    ...
    try:
        ...
        token_out, balance_out = (
            (token1, balance1) if token_in == token0 else (token0, balance0)
        )
...
def get_pool_address(token_address: str) -> str:
    ...
    pool_address = SmartContract.read(
        "base-sepolia", token_address, "poolAddress", abi=WOW_ABI
    )
